...
resource "azurerm_role_assignment" "key_vault_role_assignment" {
  count               = length(azurerm_key_vault.key_vault) > 0 ? 1 : 0  # Only assign if the Key Vault exists
  ...
  scope               = length(azurerm_key_vault.key_vault) > 0 ? azurerm_key_vault.key_vault[0].id : null  # Check if Key Vault exists
